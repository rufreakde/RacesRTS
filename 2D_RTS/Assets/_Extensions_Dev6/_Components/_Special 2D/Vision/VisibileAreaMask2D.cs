...
[AddComponentMenu("Dev6/2D/Vision Area Mask")]
public class VisibileAreaMask2D : MonoBehaviour
{
    [InfoBox("Dieses Script sollte nicht auf einem sich drehenden Object sein! Ebenfals besitzt das Script noch keine Vision Angles!")]
	public Material MaskingVisibleAreaMaterial;
	...
	// Private variables
	Mesh MaskingMesh; // Mesh for our light mesh
    [InfoBox("Die Layer die für das Raycasting benutz werden! Achtung alles was hier aufgeführt wird blockt den Sichtbereich!")]
	...
	void Start () {
		...
		MeshFilter meshFilter = gameObject.AddComponent<MeshFilter>();          // Add a Mesh Filter component to the light game object so it can take on a form
        MeshRenderer renderer = gameObject.AddComponent<MeshRenderer>();		// Add a Mesh Renderer component to the light game object so the form can become visible
		...
		renderer.sharedMaterial = MaskingVisibleAreaMaterial;		// Add this texture
		MaskingMesh = new Mesh();									// create a new mesh for our light mesh
		meshFilter.mesh = MaskingMesh;								// Set this newly created mesh to the mesh filter
		MaskingMesh.name = "Visibility Mask";                       // "Light Mesh" Give it a name
        MaskingMesh.MarkDynamic ();                                 // Optimize the Mesh for fast updates!
	}
	...
	void resetBounds(){
		Bounds b = MaskingMesh.bounds;
		b.center = Vector3.zero;
		MaskingMesh.bounds = b;
	}
			...
			if(tempVerts.Count > 0){
				...
				for(int r = 0; r<2; r++){
					...
					if(isEndpoint == true){
						...
						const float checkPointLastRayOffset= 0.0005f; // EDITED faster
	...
	void renderLightMesh(){
		...
		MaskingMesh.Clear ();
		MaskingMesh.vertices = initVerticesMeshLight;
		...
		MaskingMesh.triangles = triangles;												
		//lightMesh.RecalculateNormals();
		GetComponent<Renderer>().sharedMaterial = MaskingVisibleAreaMaterial;
	}
